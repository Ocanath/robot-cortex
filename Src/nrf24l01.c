...
NRF_RESULT nrf_send_packet(nrf24l01* dev, const uint8_t* data, uint32_t timeout_val)
{
    ...
    uint32_t tx_timeout_ts = HAL_GetTick();
    while (dev->tx_busy == 1)
    {
    	if(HAL_GetTick() > (tx_timeout_ts + timeout_val))
    		return NRF_ERROR;
